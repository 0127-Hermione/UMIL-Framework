...
class DeepZoomImageTiler(object):
    ...
    def _write_tiles(self):
        print("self._dz.level_count:", self._dz.level_count)
        print("self._target_levels",self._target_levels)
        target_levels = [self._dz.level_count-i-1 for i in self._target_levels]
        print("target_level:",target_levels)
...
class DeepZoomStaticTiler(object):
    ...
    def __init__(self, slidepath, basename, mag_levels, base_mag, objective, format, tile_size, overlap,
        ...
        self._basename = basename
        self._format = format  #.jpeg
        self._tile_size = tile_size
        self._overlap = overlap  #overlapping
        self._mag_levels = mag_levels  #patch-level
        self._base_mag = base_mag
        self._objective = objective
        self._limit_bounds = limit_bounds #limit_bounds=True
        self._workers = workers
        self._queue = JoinableQueue(2 * workers) #创建2*works个子进程


        self._dzi_data = {}
        for _i in range(workers): #子进程处理
...
def nested_patches(img_slide, out_base, level=(0,), ext='jpeg'):
    ...
    if len(level)==1:
        patches = glob.glob(os.path.join('E:\\Panda\\WSI\\WSI_temp_files', '*', '*.'+ext))
        ...
        levels.sort()
        low_patches = glob.glob(os.path.join('E:\\Panda\\WSI\\WSI_temp_files', str(levels[0]), '*.'+ext))
        for i, low_patch in enumerate(low_patches):
            ...
            for x_pos in high_x_list:
                for y_pos in high_y_list:
                    high_patch = glob.glob(os.path.join('E:\\Panda\\WSI\\WSI_temp_files', str(levels[1]), '{}_{}.'.format(x_pos, y_pos)+ext))
                    if len(high_patch)!=0:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dataset', type=str, default='E:\\Panda\\WSI\\', help='Dataset name')
    parser.add_argument('-e', '--overlap', type=int, default=0, help='Overlap of adjacent tiles [0]')
    parser.add_argument('-f', '--format', type=str, default='jpg', help='Image format for tiles')
    parser.add_argument('-v', '--slide_format', type=str, default='tiff', help='Image format for tiles')
    parser.add_argument('-j', '--workers', type=int, default=0, help='Number of worker processes to start [4]')
    parser.add_argument('-q', '--quality', type=int, default=70, help='JPEG compression quality [70]')
    parser.add_argument('-s', '--tile_size', type=int, default=256, help='Tile size')
    parser.add_argument('-b', '--base_mag', type=float, default=20, help='Maximum magnification for patch extraction')
    parser.add_argument('-m', '--magnifications', type=int, nargs='+', default=(1,), help='Levels for patch extraction')
    parser.add_argument('-o', '--objective', type=float, default=20, help='The default objective power if metadata does not present')
    parser.add_argument('-t', '--background_t', type=int, default=15, help='Threshold for filtering background')
    ...
    # print("all_slide",all_slides)
    # pos-i_pos-j -> x, y
    for idx, c_slide in enumerate(all_slides): #WSI
        print('Process slide {}/{}'.format(idx+1, len(all_slides)))
        print("c_slide:",c_slide)
        #DeepZoomStaticTiler
        DeepZoomStaticTiler(c_slide, 'E:\\Panda\\WSI\\WSI_temp_files', levels, args.base_mag, args.objective, args.format, args.tile_size, args.overlap, True, args.quality, args.workers, args.background_t).run()
