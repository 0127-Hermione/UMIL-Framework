...
def main():
    parser = argparse.ArgumentParser(description='Train AUMIL on classfical MIL datasets')
    parser.add_argument('--datasets', default='musk2', type=str, help='Choose MIL datasets from: musk1, musk2, elephant, fox, tiger [musk1]')
    ...
    for k in range(0, args.cv_fold):
        ...
        for epoch in range(0, args.num_epoch):
            ...
            optimal_auc = max(auc_value, optimal_auc)
            optimal_fscore = max(fscore, fscore)
            scheduler.step()
        print('\n Optimal accuracy: %.4f ' % (optimal_ac))
        print('\n Optimal auc: %.4f ' % (optimal_auc))
        print('\n Optimal fscore: %.4f ' % (optimal_fscore))

        acs.append(optimal_ac)
        aucs.append(optimal_auc)
        fscores.append(optimal_fscore)
    print('\n Cross validation accuracy mean: %.4f, std %.4f ' % (np.mean(np.array(acs)), np.std(np.array(acs))))
    print('\n Cross validation auc mean: %.4f, std %.4f ' % (np.mean(np.array(aucs)), np.std(np.array(aucs))))
    print('\n Cross validation fscore mean: %.4f, std %.4f ' % (np.mean(np.array(fscores)), np.std(np.array(fscores))))
